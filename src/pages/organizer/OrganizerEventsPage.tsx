...
const OrganizerEventsPage = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'DRAFT' | 'ACTIVE' | 'COMPLETED' | 'CANCEL' | 'DELETED'>('all');
  const [searchTerm, setSearchTerm] = useState('');

  const { data: events = [], isLoading, error } = useQuery({
    queryKey: ['organizerEvents', organizerId, filter, searchTerm],
    queryFn: () => getOrganizerEvents(Number(organizerId!), {
      query: searchTerm || undefined,
      eventStatus: filter === 'all' ? undefined : filter,
      page: 0,
      size: 10, // Load up to 50 events; add pagination if needed
    }),
    enabled: !!organizerId,
  });
  ...
  // Transform API data (SearchResultDTO) to match Event type
  const transformedEvents: Event[] = events.map((event: any) => ({
    ...event,
    difficultyLevel: event.difficultyLevel, // Already string (enum name)
    status: event.status, // String like "ACTIVE"
  ...
  // No client-side filtering needed (server handles query and status)
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="mb-8">
          ...
          {/* Stats (now on filtered/searched events) */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
              <div className="text-2xl font-bold text-green-600">
                {transformedEvents.filter(e => e.status === 'ACTIVE').length}
              </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
              ...
              <div className="flex gap-4 items-center">
                ...
                >
                  ...
                  {/* <option value="DELETED">Deleted</option> */} {/* Optional, if organizers can view deleted */}
                </select>
                ...
                <div className="p-4">
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-[#1E3A5F]">NPR{event.price}</span>
                    </div>
                  </div>
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="bg-[#1E3A5F] text-white px-3 py-2 rounded-lg hover:bg-[#2a4a7a] transition-colors duration-200 text-[14px]"
                      >
                        view Details
                        {/* <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          ...
                          <div className="text-sm text-gray-500">NPR{event.price} • {formatDifficulty(event.difficultyLevel)}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
